...
if __name__ == "__main__":
    ...
    DIR = Path("Codigo")
    ALMANAQUE = Path("LaTeX/Biblioteca.tex")
    ...
    if Path("LaTeX/Biblioteca.tex").exists():
        ok = os.system("rubber --pdf --inplace LaTeX/Biblioteca.tex")
        if (ok != 0):
            raise Exception("Erro ao compilar o arquivo LaTeX/Biblioteca.tex")
    else:
        raise Exception("Arquivo LaTeX/Biblioteca.tex não encontrado.")

    if Path("LaTeX/Biblioteca.pdf").exists():
        print("\nPDF gerado com sucesso e salvo em PDF/Biblioteca.pdf")
        os.replace("LaTeX/Biblioteca.pdf", "PDF/Biblioteca.pdf")
        os.replace("LaTeX/Biblioteca.tex", "PDF/Biblioteca.tex")
    else:
        raise Exception("PDF não foi gerado, ocorreu algum erro.")

    if clean:
        if Path("LaTeX/Biblioteca.log").exists():
            os.remove("LaTeX/Biblioteca.log")
        if Path("LaTeX/Biblioteca.aux").exists():
            os.remove("LaTeX/Biblioteca.aux")
        if Path("LaTeX/Biblioteca.out").exists():
            os.remove("LaTeX/Biblioteca.out")
        if Path("LaTeX/Biblioteca.toc").exists():
            os.remove("LaTeX/Biblioteca.toc")
        if Path("LaTeX/Biblioteca.tex").exists():
            os.remove("LaTeX/Biblioteca.tex")
